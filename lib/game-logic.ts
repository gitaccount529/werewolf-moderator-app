...
export function getNightSteps(gameId: number, round: number): NightStepInfo[] {
  // Get all roles in this game that have night actions
  const nightRoles = queryAll<Role & { player_ids: string }>(
  ...
  for (const role of nightRoles) {
    ...
    if (role.name === 'Apprentice Seer') {
      const seerAlive = queryOne<{ id: number }>(
...
export function resolveNight(gameId: number, round: number): NightResolution {
  const actions = queryAll<NightAction & { role_name: string; actor_name: string; target_name: string }>(
  ...
  if (wolfKill?.target_player_id) {
    ...
    if (!protected_) {
      const gameMeta = queryOne<{ metadata_json: string }>('SELECT metadata_json FROM games WHERE id = ?', gameId);
      ...
      if (meta.items && Array.isArray(meta.items)) {
        ...
        if (sandwich) {
          protected_ = true;
          sandwich.used = true;
          run('UPDATE games SET metadata_json = ? WHERE id = ?', JSON.stringify(meta), gameId);
    ...
    if (!protected_) {
      const target = queryOne<Player & { role_name: string }>(
      ...
      if (target) {
        ...
        // Special case: Cursed — becomes werewolf instead of dying
          ...
          const wolfRole = queryOne<{ id: number }>('SELECT id FROM roles WHERE name = ?', 'Werewolf');
          if (wolfRole) {
            run('UPDATE player_roles SET role_id = ? WHERE player_id = ? AND game_id = ?',
  ...
  if (cultRecruit?.target_player_id) {
    ...
    const game = queryOne<{ metadata_json: string }>('SELECT metadata_json FROM games WHERE id = ?', gameId);
  ...
  if (vampireBite?.target_player_id) {
    ...
    const targetRole = queryOne<{ team: string }>(
    ...
    if (targetRole && targetRole.team !== 'werewolf') {
      const vampireRole = queryOne<{ id: number }>('SELECT id FROM roles WHERE name = ?', 'Vampire');
      if (vampireRole) {
        run('UPDATE player_roles SET role_id = ? WHERE player_id = ? AND game_id = ?',
  ...
  transaction(() => {
    for (const death of deaths) {
      ...
      run(
        ...
        gameId,
        round,
        `${death.playerName} was killed (${death.cause})`,
    ...
    if (banished !== null || silenced !== null) {
      ...
      if (banished !== null) updates.push(`'$.banished', ${banished}`);
      if (silenced !== null) updates.push(`'$.silenced', ${silenced}`);
      run(
        `UPDATE games SET metadata_json = json_set(COALESCE(metadata_json, '{}'), '$.banished', ?, '$.silenced', ?)
         WHERE id = ?`,
        banished,
        silenced,
        gameId,
  ...
  const winResult = checkWinCondition(gameId);
  ...
  if (deaths.length > 0) {
    const gameMeta2 = queryOne<{ metadata_json: string }>('SELECT metadata_json FROM games WHERE id = ?', gameId);
    ...
    if (meta2.items_enabled && !meta2.first_night_death_occurred) {
      ...
      if (wolfDeath) {
        ...
        run('UPDATE games SET metadata_json = ? WHERE id = ?', JSON.stringify(meta2), gameId);
      }
    }
  }
...
export function checkWinCondition(gameId: number): WinResult | null {
  const alivePlayers = queryAll<Player & { role_name: string; role_team: string }>(
...
// ─── Death Chain (Phase 7 stub) ──────────────────────────────

export function resolveDeathChain(
  gameId: number,
  initialDeaths: DeathRecord[],
  round: number,
): { chainDeaths: DeathRecord[]; pendingHunterKill: boolean } {
  ...
  while (queue.length > 0) {
    ...
    const playerRole = queryOne<{ role_name: string; role_team: string }>(
    ...
    if (playerRole.role_name === 'Mad Bomber') {
      const neighbors = getAliveNeighbors(death.playerId, gameId);
      for (const neighbor of neighbors) {
        if (!processed.has(neighbor.id)) {
          const neighborRole = queryOne<{ name: string }>(
          ...
          // Kill in DB
          run('UPDATE players SET is_alive = 0, death_cause = ?, death_round = ? WHERE id = ?',
    ...
    if (lovers && lovers.includes(death.playerId)) {
      ...
      if (partnerId && !processed.has(partnerId)) {
        const partner = queryOne<Player>('SELECT * FROM players WHERE id = ? AND is_alive = 1', partnerId);
        if (partner) {
          const partnerRole = queryOne<{ name: string }>(
            `SELECT r.name FROM player_roles pr JOIN roles r ON r.id = pr.role_id
             WHERE pr.player_id = ? AND pr.game_id = ?`,
            partnerId,
            gameId,
    ...
    if (playerRole.role_name === 'Dire Wolf' && direWolfBond && !processed.has(direWolfBond)) {
      const bonded = queryOne<Player>('SELECT * FROM players WHERE id = ? AND is_alive = 1', direWolfBond);
      if (bonded) {
        const bondedRole = queryOne<{ name: string }>(
          `SELECT r.name FROM player_roles pr JOIN roles r ON r.id = pr.role_id
           WHERE pr.player_id = ? AND pr.game_id = ?`,
          direWolfBond,
          gameId,
...
function getAliveNeighbors(playerId: number, gameId: number): Player[] {
...
export function resolveLynch(
  gameId: number,
  targetPlayerId: number,
  round: number,
): { death: DeathRecord | null; princeRevealed: boolean; tannerWin: boolean } {
  const target = queryOne<Player & { role_name: string; role_team: string }>(
    `SELECT p.*, r.name as role_name, r.team as role_team
     FROM players p
     JOIN player_roles pr ON pr.player_id = p.id AND pr.game_id = ?
     JOIN roles r ON r.id = pr.role_id
  ...
  const lynchMeta = JSON.parse(
    (queryOne<{ metadata_json: string }>('SELECT metadata_json FROM games WHERE id = ?', gameId))?.metadata_json || '{}',
  );
  if (lynchMeta.items && Array.isArray(lynchMeta.items)) {
    ...
    if (charm) {
      charm.used = true;
      run('UPDATE games SET metadata_json = ? WHERE id = ?', JSON.stringify(lynchMeta), gameId);
      run(
